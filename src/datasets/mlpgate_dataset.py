    ...
    def process(self):
        ...
        for cir_idx, cir_name in enumerate(circuits):
            ...
            graph = parse_pyg_mlpgate( 
                ...
                rc_pair_index, is_rc,
                has_redundant_fault, 
                self.args.use_edge_attr, 
                self.args.reconv_skip_connection, 
                self.args.no_node_cop, 
                self.args.node_reconv, 
                self.args.un_directed, 
                self.args.num_gate_types, 
                self.args.dim_edge_feature, 
                self.args.logic_implication, 
                self.args.mask)
